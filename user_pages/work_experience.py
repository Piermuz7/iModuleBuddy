...
with st.expander("Add Work Experience"):
    ...
    with st.form("my_form", clear_on_submit=True):
        ...
        end = st.date_input("End Date", date.today(), disabled=current_occ, max_value=date.today(), min_value=date(1980, 1, 1))
        ...
        if submitted:
            if len(company) < 1 or len(position) < 1:
                ...
                add_work_experience(company, position, start, end, current_occ, part_time)
